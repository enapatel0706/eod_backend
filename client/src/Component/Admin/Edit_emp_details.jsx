...
const Edit_emp_details = (props) => {
  ...
  const getEmpData = async () => {
    ...
    const res = await axios.get(
      `${process.env.REACT_APP_BACKEND_BASE_URL}/employee`,
      {
        params: {
          emp_id: props.empId,
        },
  ...
  const getSkill = async () => {
    ...
    try {
      let res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/skills`
      );
      if (res.status == 200) {
  ...
  const getEmpSkill = async () => {
    ...
    try {
      let res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/employee/skills`,
        {
          params: {
            emp_id: props.empId,
          },
    ...
    try {
      if (tValue != "") {
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/employee/skill`,
          {
            emp_id: props.empId,
            skill_name: tValue,
  ...
  const getProject = async () => {
    ...
    try {
      var res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/admin/projects`,
        {
          params: {
            emp_id: props.empId,
          },
  ...
  const getEmpProject = async () => {
    ...
    try {
      let res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/employee/project`,
        {
          params: {
            emp_id: props.empId,
          },
  ...
  const handleProjectStatus = async (project_id) => {
    setLoader(true);
    try {
      setLoader(false);
      Swal.fire({
        ...
        (async () => {
          ...
          if (result.isConfirmed) {
            const res = await axios.patch(
              `${process.env.REACT_APP_BACKEND_BASE_URL}/employee/project/status`,
              {
                emp_id: props.empId,
                status: "inactive",
  ...
  const handleProject = async (e) => {
    ...
    try {
      ...
      if (
        pValue.project_name == "" ||
        pValue.project_name == "-- Select Project --"
      ) {
        setLoader(false);
        Swal.fire({
          type: "error",
        ...
        if (res.status == 200 || res.status == 204) {
          ...
          if (res.data) {
            res.data.map((dt) => {
              return dt.project_id == pValue.project_id ? (temp = true) : "";
            });
                              ...
                              <div className="row col-12 mx-0 px-0 mb-3">
                                ...
                                <div className="col-12 col-sm-4 mt-3 mt-sm-0">
                                  ...
                                  <select
                                    className="form-select"
                                    name="emp_type"
                                    value={empData.emp_type}
                                    defaultValue={empData.emp_type}
                                    onChange={getData}
                                  >
                            ...
                            <div className="skill">
                              <Stack style={{ width: "100%" }}>
                                <Autocomplete
                                  ...
                                  id="free-solo-2-demo"
                                  disableClearable
                                  options={allSkill.map(
                                    (option) => option.skill_name
                                  )}
                                  ...
                                  <div className="col-12 col-sm-6 col-md-12 Mentor mt-2 mt-sm-0 mt-md-2">
                                    ...
                                    >
                                      ...
                                      {mentorList.map((data, index) => {
                                        return data.emp_id != props.empId &&
